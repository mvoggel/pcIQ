...
async def fetch_adv_data(crd: str, timeout: float = 12.0) -> ADVData | None:
    ...
    _MAX_PDF_BYTES = 15 * 1024 * 1024  # 15 MB — covers ~98% of ADV PDFs
    # Memory is safe: background task is sequential + we only parse 15 pages (not the full doc)
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            if len(resp.content) > _MAX_PDF_BYTES:
                return None  # genuinely giant PDF (>15MB) — skip
